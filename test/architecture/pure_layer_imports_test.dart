...
void main() {
  ...
  test(
    'repositories never import use_cases (the dependency points one way)',
    () {
      final offenders = _scan(
  ...
  test(
    'ViewModels hold no Timer / Navigator / BuildContext (the View owns those)',
    () {
      ...
      // matching nothing.
      final viewmodelFiles = _dartFilesUnder([
        'lib/ui',
        'lib/kanji/ui',
      ]).where((f) => f.path.endsWith('viewmodel.dart')).toList();
      expect(
        viewmodelFiles,
        isNotEmpty,
        reason:
  ...
  test(
    'the UI layer never touches platform packages directly (services wrap them)',
    () {
      final offenders = _scan(
